...
<div class="grid-container">
    ...
    if (isset($_GET['pubchem_id'])) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            $model_path = $data['model_path']; // Ensure this field is present in your database
            $image_compound = $data['image_blob']; // Get the image path
        ...
        //echo '<div id="3dviewer" class="viewer" style="width: 300px; height: 300px;"></div>'; 
        // Display image if available
        if (!empty($image_compound)) {
            echo "<img src='{$image_compound}' alt='Structure Image' style='width: 100%; max-width: 300px; margin-top: 20px;'>";
        } else {
            echo "<p>No image available</p>";
        }

        echo '</section>';
        // Pass the PubChem ID and model path to JavaScript
        echo "<script>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        if (modelPath) {
            var stage = new NGL.Stage("3dviewer");
            stage.loadFile(modelPath, { defaultRepresentation: true }).then(function(o) {
                o.autoView();
            });
        }
    });
